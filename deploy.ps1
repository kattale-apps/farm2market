...
try {
    ...
    Write-Host "WARNING: Node.js is not installed. Please install Node.js 18+ first." -ForegroundColor Yellow
    exit 1
}
...
try {
    ...
    Write-Host "WARNING: Git is not installed. Please install Git first." -ForegroundColor Yellow
    exit 1
}
...
Write-Host "Make sure you are logged in to Convex: npx convex login" -ForegroundColor Yellow
Read-Host "Press Enter to continue with Convex deployment"
npx convex deploy --yes
Write-Host "Convex deployment complete" -ForegroundColor Green
Write-Host ""
Write-Host "IMPORTANT: Save your Convex deployment URL for Vercel configuration" -ForegroundColor Yellow
Write-Host ""

# Step 7: Deploy to Vercel
Write-Host "Step 7: Deploying to Vercel..." -ForegroundColor Blue
Write-Host "Make sure you are logged in to Vercel: vercel login" -ForegroundColor Yellow
...
if ($push -eq "y" -or $push -eq "Y") {
    try {
        ...
        Write-Host "WARNING: No GitHub remote configured or push failed" -ForegroundColor Yellow
