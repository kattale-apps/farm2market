...
type SectionData = Partial<NonNullable<Doc<"agroFreshUGFarmValidations">["section2_8_maize"]>>;
...
export function Section2_8_Maize({ data, onUpdate, readOnly, preloadedWaterSource }: Props) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div>
        <label style={{ fontWeight: 600 }}>Production intensity (tons per acre)</label>
        <input
          type="text"
          value={data?.productionIntensityPerAcre ?? ""}
          onChange={(e) => handleChange("productionIntensityPerAcre", e.target.value)}
          placeholder="e.g., 10–15 tons per acre"
          style={inputStyle}
          disabled={readOnly}
        />
      </div>
