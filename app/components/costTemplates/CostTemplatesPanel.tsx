...
function CreateTemplateForm({ userId, type, communityId, onDone }: {
  userId: Id<"users">;
  type: Tab;
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      // Inside a community dashboard the template belongs to that community;
      // the standalone admin page keeps creating system-wide templates.
      const ownership = communityId
        ? { ownerType: "community" as const, communityId }
        : { ownerType: "system" as const };
      if (type === "crop") {
        await createCrop({ ownerId: userId, ...ownership, cropType: name, emoji, acreSize, stages, currency: "UGX" });
      } else {
        await createLivestock({ ownerId: userId, ...ownership, livestockType: name, emoji, acreSize, stages, currency: "UGX" });
      }
      onDone();
    } catch (e: any) {
...
// ─── MAIN PANEL ───────────────────────────────────────────────────────────────
/**
 * Cost template manager. Rendered on its own admin page, and as the "Cost
 * Templates" tab of a community dashboard (embedded, scoped to that community).
 */
export function CostTemplatesPanel({ userId: userIdProp, communityId, embedded = false }: {
  userId?: Id<"users"> | null;
  communityId?: Id<"communities">;
  embedded?: boolean;
} = {}) {
  const { user } = useStoredUser();
  const userId = userIdProp ?? ((user?.userId as Id<"users"> | undefined) ?? null);
  const [activeTab, setActiveTab] = useState<Tab>("crop");
  const [mode, setMode] = useState<Mode>("list");
  const [calcTarget, setCalcTarget] = useState<any | null>(null);

  const templateArgs = userId
    ? (communityId ? { requestingUserId: userId, communityId } : { requestingUserId: userId })
    : "skip";
  ...
  return (
    <div style={{ fontFamily: FONT, minHeight: embedded ? undefined : "100vh", background: embedded ? undefined : "#f4f6f9", paddingBottom: embedded ? 0 : 80 }}>
      {!embedded && (
      ...
      <div style={{ padding: "clamp(1rem,4vw,1.5rem)", maxWidth: embedded ? undefined : 800, margin: "0 auto" }}>
        {mode === "list" && (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
        ...
        {mode === "create" && userId && (
          <CreateTemplateForm userId={userId} type={activeTab} communityId={communityId} onDone={() => setMode("list")} />
        )}
      </div>
