...
type SectionData = Partial<NonNullable<Doc<"agroFreshUGFarmValidations">["section2_6_aquaculture"]>>;
...
export function Section2_6_Aquaculture({ data, onUpdate, readOnly }: Props) {
  ...
  const fishOptions = ["Tilapia", "Catfish"];
  const toggleFish = (value: string) => {
    const current = data?.typeOfFish ?? [];
    const next = current.includes(value)
      ? current.filter((v) => v !== value)
      : [...current, value];
    handleChange("typeOfFish", next);
  };

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <h2>2.6 Aquaculture</h2>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "0.5rem" }}>
          ...
          <input
            type="number"
            min="0"
            value={data?.enterpriseAreaDepth ?? ""}
            onChange={(e) => handleChange("enterpriseAreaDepth", e.target.value)}
            placeholder="Depth"
            style={inputStyle}
            disabled={readOnly}
          />
        </div>
        <input type="text" value={data?.enterpriseAreaAcres ?? ""} placeholder="Calculated acres" style={inputStyle} disabled />
      </div>

      <div>
        <label style={{ fontWeight: 600 }}>Current stock (fingerlings)</label>
        <input
          type="number"
          min="0"
      ...
      <div>
        <label style={{ fontWeight: 600 }}>Type of fish</label>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "0.5rem" }}>
          {fishOptions.map((option) => (
            <label key={option} style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
              <input
                type="checkbox"
                checked={(data?.typeOfFish ?? []).includes(option)}
                onChange={() => toggleFish(option)}
                disabled={readOnly}
              />
              <span>{option}</span>
            </label>
        ...
        <input
          type="text"
          value={data?.typeOfFishOther ?? ""}
          onChange={(e) => handleChange("typeOfFishOther", e.target.value)}
          placeholder="Other fish types"
          style={inputStyle}
          disabled={readOnly}
        />
      </div>
