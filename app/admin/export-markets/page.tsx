...
import { ugandaDateFromInstant, expiryState } from "../../../convex/exportMarketsShared";
import {
  FONT,
  ON_PHOTO_SHADOW,
  EXPORT_BROWN,
  ...
  return (
    <div style={{ padding: "1rem", maxWidth: 960, margin: "0 auto", fontFamily: FONT }}>
      ...
      {mine && mine.communities.length === 0 && (
        <Notice tone="info">
          No exporter community is set up yet. A super admin turns on &quot;Export Markets&quot; for a community in the
          community dashboard.
        </Notice>
      )}
      ...
      <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap", marginBottom: "1rem" }}>
        {tabs.map(([key, text]) => (
          <button key={key} onClick={() => setTab(key)} style={{ ...button(tab === key ? "primary" : "secondary"), fontSize: "0.82rem" }}>
            {text}
          </button>
        ))}
      </div>
...
type P = { adminId: Id<"users">; today: string; setMsg: (m: Msg) => void };

function Applications({ adminId, today, setMsg }: P) {
  const [status, setStatus] = useState<"submitted" | "approved" | "rejected" | "suspended" | "draft">("submitted");
  const rows = useQuery(api.exportMarkets.listExporterProfilesForReview, { adminId, status, today });
...
function Documents({ adminId, today, setMsg }: P) {
  const docs = useQuery(api.exportMarkets.listDocumentsForReview, { adminId });
...
function Members({ adminId, today, communities, setMsg }: P & { communities: { _id: Id<"communities">; name: string }[] }) {
...
function DocumentTypes({ adminId, setMsg }: { adminId: Id<"users">; setMsg: (m: Msg) => void }) {
  ...
  const saveRow = async (row: { key?: string; label: string; description?: string; required: boolean; hasExpiry: boolean; isActive: boolean }) => {
    setBusy(true);
    try {
