...
type SectionData = Partial<NonNullable<Doc<"agroFreshUGFarmValidations">["section2_2_poultry"]>>;
...
export function Section2_2_Poultry({ data, onUpdate, readOnly }: Props) {
  ...
  const chickenOptions = ["Broiler", "Layer", "Offlayer", "Kroiler", "Local"];
  const toggleChicken = (value: string) => {
    const current = data?.typesOfChicken ?? [];
    const next = current.includes(value)
      ? current.filter((v) => v !== value)
      : [...current, value];
    handleChange("typesOfChicken", next);
  };

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <h2>2.2 Poultry Farming</h2>
      ...
      <div>
        <label style={{ fontWeight: 600 }}>Current Poultry intensity</label>
        <input
          type="number"
          min="0"
          value={data?.currentPoultryIntensity ?? ""}
          onChange={(e) => handleChange("currentPoultryIntensity", e.target.value)}
          style={inputStyle}
          disabled={readOnly}
        />
      </div>
      ...
      <div>
        <label style={{ fontWeight: 600 }}>Types of Chicken</label>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "0.5rem" }}>
          {chickenOptions.map((option) => (
            <label key={option} style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
              <input
                type="checkbox"
                checked={(data?.typesOfChicken ?? []).includes(option)}
                onChange={() => toggleChicken(option)}
                disabled={readOnly}
              />
              <span>{option}</span>
            </label>
        ...
        <input
          type="text"
          value={data?.typesOfChickenOther ?? ""}
          onChange={(e) => handleChange("typesOfChickenOther", e.target.value)}
