...
/**
 * Update existing communities with their logo paths
 * Maps community names to their corresponding logo files in /public
 */
export const updateCommunitiesWithLogos = mutation({
  args: {
    adminId: v.id("users"),
  },
  handler: async (ctx, args) => {
    ...
    for (const community of allCommunities) {
      const communityName = community.name?.toUpperCase() || "";
      let logoPath: string | undefined;

      // Check for exact match
      for (const [name, path] of Object.entries(logoMapping)) {
        if (name.toUpperCase() === communityName) {
          logoPath = path;
          break;
        }
      }

      // If community doesn't have a logo path and we found one in mapping
    ...
    return {
      ...
      updatedCount,
      results,
      message: `Updated ${updatedCount} communities with logos`,
    };
  },
});
