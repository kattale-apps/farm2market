...
export default function AdvancePurchaseAdminPage() {
  ...
  const configs = useQuery(
    api.advancePurchase.listConfigsForCommunity,
      ...
      {selectedCommunityId && (
        <>
          ...
          {tab === "configure" && (
            <>
              {showBuilder ? (
                <ConfigBuilder adminId={userId!} communityId={selectedCommunityId} onDone={() => setShowBuilder(false)} />
              ) : (
          ...
          {tab === "review" && <EvidenceReview adminId={userId!} communityId={selectedCommunityId} />}
        </>
      )}
    </div>
  );
