...
/**
 * Crop and animal check (Diagnostics, free option). Shown inside a community
 * (/community-only/diagnose) and from the farmer dashboard (/farmer/diagnose).
 * Any farmer can use it; a community, when given, records where the check
 * was made and enables that community's paid AI photo check.
 *
...
export default function CropCheck({
  ...
  // "animals" opens the livestock check; anything else is the crop check.
  const group: HostGroup = kind === "animals" ? "livestock" : "crop";
  ...
  const library = useOfflineQuery(
    ...
    userId ? { userId, communityId: communityId ?? undefined } : "skip"
  );
  ...
  const sendDraft = useCallback(
    async (draft: DiagnosticDraftEntry) => {
      ...
      const saved = await saveReport({
        ...
        communityId: (draft.communityId || undefined) as Id<"communities"> | undefined,
  ...
  const runCheck = async () => {
    ...
    const draft: DiagnosticDraftEntry = {
      ...
      communityId: communityId ? String(communityId) : "", // "" = checked outside a community
      host,
      ...
      <div style={{ padding: "1rem", maxWidth: 560, margin: "0 auto" }}>
        ...
        {library === undefined ? (
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: "1.25rem", textAlign: "center" }}>
            <p style={{ fontSize: "1rem" }}>Crop and animal checks are for farmer accounts.</p>
